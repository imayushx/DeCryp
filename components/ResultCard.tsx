...
import type { AIExplanation, PreSignExplanation, PostSignExplanation } from "@/lib/ai";
...
// ─── Action buttons ───────────────────────────────────────────────────────────
function ActionButtons({ mode, onReset }: { mode: "pre-sign" | "post-sign"; onReset: () => void }) {
  if (mode === "pre-sign") {
    return (
...
export default function ResultCard({ decoded, explanation, raw, onReset }: ResultCardProps) {
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="w-full max-w-[640px] mx-auto space-y-3">
        ...
        <Panel delay={0.44}>
          <div className="px-6 py-5">
            ...
            <ActionButtons mode="pre-sign" onReset={onReset} />
          </div>
        </Panel>
      </motion.div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="w-full max-w-[640px] mx-auto space-y-3">
      ...
      <RawDetails decoded={decoded} raw={raw} open={rawOpen} onToggle={() => setRawOpen(v => !v)} />
      <ActionButtons mode="post-sign" onReset={onReset} />
    </motion.div>
  );
}
